...
<?php $itemid_for_checkout = $this->config->get('checkout_itemid', '0'); ?>
<?php if (!empty($itemid_for_checkout)) {
...
<?php $this->setLayout('listing_price');
...
<?php $cart_type = $this->params->get('cart_type', 'cart'); ?>
<?php if ($this->params->get('from', 'no') == 'no') {
...
<?php if (empty($this->rows)) {
    ...
    if (!empty($desc) || $desc == '0') {
        ...
        <div id="hikashop_cart" class="hikashop_cart">
            <?php $text = '<span class="fa-stack fa-1x"><i class="fa fa-shopping-cart fa-stack-1x"></i></span>';
            echo $this->cart->displayButton($text, 'checkout', $this->params, $url_checkout, 'window.location=\'' . $url_checkout . '\';return false;', '', 0, 1, 'oe-menu-cart oe-circle-border');?>
        </div>
        <div class="clear_both"></div>
    <?php } ?>
<?php } else { ?>
    <div id="hikashop_cart" class="hikashop_cart">
    <?php if ($this->config->get('print_cart', 0) && JRequest::getVar('tmpl', '') != 'component' && $cart_type != 'wishlist') { ?>
    ...
    if ($this->params->get('small_cart')) {
        ...
        if ($cart_type != 'wishlist') {
            ...
            if ($this->params->get('show_cart_proceed', 1) && $this->params->get('cart_type', 'cart') != 'wishlist' && $this->params->get('from', 'display') == 'module') {
                $text = '<span class="fa-stack fa-1x"><i class="fa fa-shopping-cart fa-stack-1x"></i></span>';
                echo $this->cart->displayButton($text, 'checkout', $this->params, $url_checkout, 'window.location=\'' . $url_checkout . '\';return false;', '', 0, 1, 'oe-menu-cart oe-circle-border');
            }
        } else {
            ...
            <a class="hikashop_small_cart_checkout_link"
               href="<?php echo hikashop_completeLink('cart&task=showcart&cart_id=' . $cart_id . '&cart_type=' . $cart_type . $url_itemid); ?>">
                    ...
                    switch ($row_count) {
                        ...
                        case 1:
                            ...
                            <td class="hikashop_cart_module_coupon_value">
                                <?php if (!$this->params->get('price_with_tax')) {
