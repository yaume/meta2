...
<?php
	if($this->image->override) {
		...
		if(@$img->success) {
			...
			$html = '<img id="hikashop_main_image'.$variant_name.'"class="img-fluid main-img" title="'.$this->escape(@$image->file_description).'" alt="'.$this->escape(@$image->file_name).'" src="'.$img->url.'"/>';
...
<?php
	if( !empty($this->element->images) && count($this->element->images) > 1) {
		...
		foreach($this->element->images as $image) {
			...
			if($this->image->override) {
				echo $this->image->display($image->file_path, 'hikashop_main_image'.$variant_name, $image->file_name, 'class="hikashop_child_image img-fluid"','', $width,  $height);
				continue;
			}
			...
			$id = null;
			$classname = 'hikashop_child_image img-fluid';
			if($firstThunb) {
